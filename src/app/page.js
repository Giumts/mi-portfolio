...
export default function Home() {
  ...
  const projects = [
    ...
    // Si tienes más proyectos, añádelos aquí con id: "p2", "p3"...
  ];
          ...
          <motion.div key="projects" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={{ position: "relative", width: "100vw", height: "100vh", overflow: "hidden" }}>
            {projects.map((proj, index) => (
              <motion.div 
                ...
                style={{ position: "absolute", top: randomPositions[index]?.top, left: randomPositions[index]?.left, rotate: randomPositions[index]?.rotation, width: "100px", zIndex: 50 }}
              >
                <motion.img src={proj.img} whileHover={{ scale: 1.1 }} style={{ width: "100%", filter: "grayscale(100%)", transition: "filter 0.4s" }} onMouseOver={e => e.currentTarget.style.filter="grayscale(0%)"} onMouseOut={e => e.currentTarget.style.filter="grayscale(100%)"} />
              </motion.div>
            ))}
          </motion.div>
        )}

        {/* 3. DETALLE DE CADA PROYECTO */}
        {selectedProject && view === selectedProject.id && (
          <motion.div key="detail" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={{ width: "100vw", paddingTop: "0" }}>
            {/* Portada Recortada */}
            ...
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "15vh", paddingBottom: "20vh" }}>
              {selectedProject.gallery.map((url, i) => (
                <motion.img 
                  ...
                  initial={{ opacity: 0 }}
                  onClick={(e) => {
                    e.currentTarget.style.width = e.currentTarget.style.width === "85vw" ? "45vw" : "85vw";
                  }}
